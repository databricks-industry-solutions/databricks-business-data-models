...
Separate from launch_e2e_v488.py so a dead launcher shell never orphans the monitoring
...
PROFILE = "my-uae"
RUN_ID = "564741857926303"
CATALOG = "vibe_e2e_v488"
BUSINESS = "coffee_roastery"
MIRROR = "/tmp/e2e488_logs"
PULSES = "/tmp/e2e488_pulses.txt"
...
def scan():
    hits = {}
    for pat, label in (
        ("v488-colname-rename-sync FIRED", "v488 stale column_name resynced"),
        ("canonical-reconcile DRIFT", "physical/memory column drift"),
        ("mv-column-rename-before-prune FIRED", "v488 rename instead of prune"),
        ("mv-column-prevalidate-prune FIRED", "column pruned (rename impossible)"),
        ("mv-inflight-repair-persist FIRED", "v488 repair captured"),
        ("mv-artifact-mirrors-executed FIRED", "v488 artifact rewritten"),
