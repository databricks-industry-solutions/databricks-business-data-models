...
Separate from launch_e2e_v490.py so a dead launcher shell never orphans the monitoring
...
PROFILE = "my-uae"
RUN_ID = str(json.load(open("/tmp/e2e490_run.json"))["run_id"])
CATALOG = "vibe_e2e_v490"
BUSINESS = "coffee_roastery"
MIRROR = "/tmp/e2e490_logs"
PULSES = "/tmp/e2e490_pulses.txt"
...
def scan():
    ...
    for pat, label in (
        ...
        ("shrink-guard-user-king FIRED", "v490 shrink-guard user-king"),
        ...
        ("mv-inflight-repair-persist FIRED", "v490 repair captured"),
        ("mv-artifact-mirrors-executed FIRED", "v490 artifact rewritten"),
