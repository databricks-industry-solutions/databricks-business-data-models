...
| --- | --- | --- |
| **(a) Deliverable ready + self-graded** | A batch/phase completed and you have graded it | **Report the mini-scorecard and PROCEED.** State results; do not ask permission to move to the next batch. Only stop here if the phase is the *last* one or the user set an explicit checkpoint. |
...
| **Authoring notebooks in bulk** | **max 4 per batch** | **Run + verify the batch before authoring the next (see Batching Discipline)** |
...
**The rule: never author more than 4 un-executed notebooks at once.**

```
For each batch of ≤ 4 notebooks (grouped by load-order tier):
  1. Author the batch (validate each notebook's SQL in scratchpad FIRST — Golden Rule)
  2. Run every notebook in the batch (or its key assertions via executeCode)
  3. Confirm results landed (PENDING rows written / target rows non-zero / no errors)
  4. Fix any failures now — do NOT proceed with known-broken notebooks in the batch
  5. Emit the batch mini-scorecard (HITL gate (a)) and PROCEED to the next batch
```
...
- **Author → run → verify is one interleaved loop, not two phases.** Do not "generate all,
  then run all." Generate a batch, prove it, move on.
...
| `domain-documentation` | `docs/.pipeline/state/run/documentation_state.md` | `NOT_STARTED→ENRICHED→VALIDATED` | One Genie space, created in Finalize; parallel value low |
