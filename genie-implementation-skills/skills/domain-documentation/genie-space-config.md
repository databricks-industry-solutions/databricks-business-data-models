...
createAsset(
  asset={
    "assetType": "genie",
    "name": "{Domain} Analytics",  # e.g., "Manufacturing V2 Analytics"
    "tableIdentifiers": [
...
- Name follows pattern: `{Domain} Analytics` (e.g., "Manufacturing V2 Analytics")
- **`createAsset` may append a timestamp to the space name** — a call with
  `name="Sales Order Analytics"` created the space as "Sales Order Analytics 2026-08-09 04:14:59".
...
- **Emit the UI rename as a pending manual step, don't block on it.** Renaming the space to the clean
  `{Domain} Analytics` pattern is a manual Genie-UI action; list it in the run's final "Pending manual
  steps" output rather than treating it as an automated, verifiable step. The docs already resolve via
  UUID, so a not-yet-renamed space does not break any reference.

### Hybrid / multi-schema variant (`output_model: hybrid`)

A `hybrid` model has tables in **two schemas** (normalized silver + dimensional gold). The Genie space
includes tables from **both**:
- `tableIdentifiers` = all gold dims + all gold facts (from `{gold_catalog}.{gold_schema}`) **plus** all
  silver tables (from `{silver_catalog}.{silver_schema}`), EXCLUDING `_validation_*` in both schemas.
- The instruction text must teach the two-layer pattern so Genie routes questions correctly (see
  Priority 1 below): **the gold star is the preferred analytics surface** (clean star joins, conformed
  dims); **silver 3NF is for operational detail, lineage, or entities gold doesn't model**. Without this,
  Genie will mix silver natural keys and gold surrogate keys in the same query.
...
> See `SKILL.md` Phase 5 step 3 for the per-`output_model` breakdown. The rest of this section
...
Source the description from:
1. DDL notebook (if COMMENT was defined but not applied)
2. S2T mapping (column mapping descriptions)
3. Domain narrative (entity stories)
4. Column name inference (last resort: `Plant_Key` → "Foreign key to dim_plant")
