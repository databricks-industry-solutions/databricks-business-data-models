...
> **Building gold in SDP mode?** If `etl_type: sdp_pipeline`, the **authoritative gold recipe is the
> "Hybrid mode: the downstream gold star layer" section of `sdp-pipeline-development.md`** — gold
> objects are declarative MVs in the same pipeline as silver, schema-qualified to the gold schema, not
> `INSERT OVERWRITE` marts. Also apply that file's **inline schema type discipline** (DECIMAL CAST,
> TIMESTAMP-vs-DATE, serverless `WEEKDAY()+1` instead of `DAYOFWEEK_ISO`, source-format probe) and the
> **FK-resolution fan-out** rule to every gold object — those are the failure modes that most often
