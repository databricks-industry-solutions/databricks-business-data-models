...
When `conventions.yml` `etl_type: sdp_pipeline`, the bundle emits ONE `pipeline` resource
instead of a daily `job` — the full template + parameters wiring lives in
`sdp-pipeline-development.md` "Pipeline DAB resource". Key deploy-doc points:

- **`channel: PREVIEW` + `continuous: false`** (triggered) — required for the LDP unit-test framework.
- **Parameterize with the native `parameters:` block**, resolved from DAB `variables` per target —
  NOT `base_parameters` widgets (that is the `job`/notebook_task path).
...
- **Declarative sources live under `src/silver/pipeline/`** (one notebook per entity), pulled in via
  a `libraries: [glob: {include: ../src/silver/pipeline/**}]` entry.
