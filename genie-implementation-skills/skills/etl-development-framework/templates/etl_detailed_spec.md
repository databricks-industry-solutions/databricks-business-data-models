...
*Fill for any entity where source → target mapping is already known.*
...
### [Entity name]
| Target column | Type | Nullable | Source table | Source column | Transform / cast | Notes |
| --- | --- | --- | --- | --- | --- | --- |
| | | | `{catalog}.{schema}.{table}` | `{verified_bronze_col}` | | |
...
### FK Resolution — join attribute per FK (REQUIRED for every fact FK)
...
| Fact entity | FK column | Fact source column | Dim to join | Dim join attribute | Values verified equal? |
| --- | --- | --- | --- | --- | --- |
| | {Dim}_Key | e.g. `Plant` = "SEDC" (name) | dim_plant | `Plant_Name` (NOT `Plant_Code`) | yes — sampled both |
