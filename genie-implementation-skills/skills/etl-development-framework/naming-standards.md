...
Every column in every DDL-generated table **must** have a `COMMENT`.

The table itself must also carry a `COMMENT` describing its grain:

```sql
COMMENT 'Silver dimension. One row per active employee. Source: SAP SuccessFactors odata_hr_curated.';
```

For facts: `COMMENT 'Silver fact. GRAIN: one row per {explicit grain statement}.'`
