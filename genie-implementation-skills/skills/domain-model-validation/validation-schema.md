...
CREATE TABLE IF NOT EXISTS _gap_registry (
  ...
  Gap_Type                 STRING       NOT NULL  COMMENT 'Category: MISSING_SOURCE | FK_ORPHAN | PARTIAL_COVERAGE | CONSTRAINT_RELAXED | SYNTHETIC_KEY | DEFERRED_ENRICHMENT',
...
the parallel job (see SKILL.md "Deployment Architecture: PENDING→Claim Pattern" and Rules
15/16). All SQL below is the `sql` shape; under `etl_language: python` these are `# MAGIC %sql`
...
`v_run_id` so no column can ever match, and reference `session.v_run_id`.) See SKILL.md Critical
Databricks SQL Pitfalls §6.
...
`run_id` / `run_ts` are session variables (SKILL.md "DECLARE VARIABLE Scope") available to
...
> DDL seed). The `SELECT … UNION ALL` form below evaluates them per-row and works. See SKILL.md
