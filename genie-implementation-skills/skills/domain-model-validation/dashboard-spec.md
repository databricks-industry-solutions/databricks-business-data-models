...
> `{Domain} Validation Quality` (e.g. "Manufacturing V2 Validation Quality"). The four tabs are
...
**Name:** `{Domain} Validation Quality` (e.g., "Manufacturing V2 Validation Quality")
...
1. **Run the schema self-check gate** (SKILL.md Phase 5, Step 0). Abort and report on mismatch.
...
4. **Materialize as a persistent dashboard ASSET** named `{Domain} Validation Quality` — either
   import the token-swapped JSON directly (preferred, if an import primitive exists) or reproduce
   it via `createAsset(assetType='dashboard')` → `openAsset` → `editDataset` + `simpleCreateWidget`.
