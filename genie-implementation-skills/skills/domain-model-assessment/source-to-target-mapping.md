...
**FK resolution** (for facts): {per FK — fact source column → dim + dim join attribute; note
name-vs-code / internal-vs-external mismatches. This becomes spec Section 3's FK-resolution table.}
...
| `Gap?` | `No` / `Partial` (exists but incomplete) / `Derived` (computed, no single source col) / **`GAP`** (no source) |
...
Save the S2T mapping report as:
```
{project_folder}/{YYYY-MM-DD} - {Domain} Vibe {Version} - Source-to-Target Mapping Report.md
```
Example: `meridian_sales_order/2026-07-12 - Sales Order Vibe V1 - Source-to-Target Mapping Report.md`

Use the `templates/s2t_map.md` shell to structure the output.
