...
This is the **return leg of the iterate-upstream disposition** (`discovery-protocol.md` Step
...
### Trigger F — Architecture Decision
...
| P2 | **Model addition** (bronze→model) | *(new)* `lot_genealogy` | parent/child lot links | `mtl_lot_relationships` present, no model entity | Propose new entity; optional vibe-model prompt | OPEN |
...
- **Bronze-reveals-more surfaces as a `Model addition` row** (proposed new entity / attribute /
  grain / hierarchy level) for the DE team's review — optionally paired with a vibe-model prompt
  (a byproduct, not a gate; most teams action it locally rather than re-running the agent).
...
| Section 2 — Column Mappings | From S2T column mapping tables — one subsection per entity. Target column names use `naming.business_column_case` (e.g. `Pascal_Snake` → `Net_Amt`); metadata/audit columns use `naming.metadata_column_case` (e.g. `_source_system`). |
| Section 3 — Keys & Constraints | From S2T key derivation: natural key, PK, FK references, value_regex → CHECK. **Keys per `output_model`:** `dimensional` (+ hybrid-gold) → surrogate `{Entity}_Key` (`naming.surrogate_key_suffix`), SHA2 formula, FK per `naming.fk_naming`. `normalized` (+ hybrid-silver) → **follow the model's PK** (`{product}_id`), no `{Entity}_Key` surrogate; add one only where the model PK is composite/mutable or cross-source integration needs it. Set per-entity `SCD` (type_2 dims — dimensional/hybrid-gold only) in §5. |
...
4. Tell the user:
   > "Handoff documents generated. Both satisfy the ETL framework requirements gate.
...
- Grade=Full entities have complete column mappings in Section 2
- Grade=Partial entities have column mappings with explicit GAP markers for unmapped columns
