...
Here's the brief filled in for the Meridian `sales_order` domain (`hybrid`), the same domain captured in
[`examples/sales_order_sdp_full_run/`](../../../examples/sales_order_sdp_full_run/). It's deliberately
realistic: most requirements are supported, and **two are not** — shown as gaps, not dropped.
...
Everything else maps cleanly, which is why the captured example built **5 facts, 7 dims, 1 bridge** and
reported **12 / 14 KPIs structurally supported, 2 blocked by P0 gaps** — see its
[`docs/design/gold_layer_assessment.md`](../../../examples/sales_order_sdp_full_run/docs/design/gold_layer_assessment.md).
