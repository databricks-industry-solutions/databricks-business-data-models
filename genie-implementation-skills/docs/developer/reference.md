...
| [`sales_order_sdp_full_run/`](../../examples/sales_order_sdp_full_run/) | `sdp_pipeline` / `hybrid` | **Real captured run**, frozen snapshot | Browse a complete, real six-artifact run of the *other* flow (30 tables: 17 silver + 13 gold). Start at its [`ARCHITECTURE.md`](../../examples/sales_order_sdp_full_run/ARCHITECTURE.md). |

> The `sales_order_sdp_full_run/` capture is **reference-only and dated** (skills as of 2026-08-09):
> its directory layout follows the current three-tier convention, but its content reflects that
> date and the model isn't self-contained, so it's for browsing — not reproducing. It's the
> concrete referent for the "sales-order run" cited as a hardening example across the skill files.
