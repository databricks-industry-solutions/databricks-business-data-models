...
- **See it at realistic scale.** `field_service` is the fast 5-entity loop; the
  [`examples/sales_order/`](../../examples/sales_order/README.md) domain is the full 27-table
  counterpart (a gold layer, cross-domain divergences, SQL-reserved-word edge cases) — the same
  loop, at the scale of a real customer model.
